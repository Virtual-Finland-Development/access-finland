...
import { getCachingStagedSecret } from './secrets-store';

export async function encryptUsingBackendSecret(obj: object): Promise<string> {
  const publicKey = await getCachingStagedSecret('BACKEND_SECRET_PUBLIC_KEY');
...
export async function decryptUsingBackendSecret(encryptedData: string) {
  const privateKey = await getCachingStagedSecret('BACKEND_SECRET_PRIVATE_KEY');
...
export async function generateCSRFToken() {
  const key = await getCachingStagedSecret('BACKEND_HASHGEN_KEY');
