...
export default function ResidencePermitsPage() {
  ...
  const [permits, setPermits] = useState<
    { id: number; issuer: string; type: string; status: string }[] | undefined
  >();
                  ...
                  <Expander key={permit.id}>
                    ...
                    <ExpanderContent className="!text-base !flex !flex-col !gap-4">
                      <div className="flex flex-col gap-4">
                        <Text>1. Issuer: {permit.issuer}</Text>
                        <Text>2. Type: {permit.type}</Text>
                        <Text>3. Status: {permit.status}</Text>
                      </div>
                      {permit.status === 'rejected' && (
                        <InlineAlert status="error" labelText="Rejected">
                          <Text>
                            The residence permit application has been rejected.
                          </Text>
                        </InlineAlert>
                      )}
                      <InlineAlert labelText="Permit type">
                        ...
                        <ul className="list-outside list-disc ms-8">
                          <li>
                            <Text>A = continuous residence permit</Text>
                          </li>
                          <li>
                            <Text>B = temporary residence permit</Text>
                          </li>
                          <li>
                            <Text>P = permanent residence permit</Text>
                          </li>
                          <li>
                            <Text>
